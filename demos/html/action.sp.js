function processMessage(val) {
    ...
    // Webpack-ed to embed and use here in V8!
    ...
    const debug = (obj) => sp.MessageBox(`${JSON.stringify(Object.values(obj)[0],null,2)}`, Object.keys(obj)[0]);
    ...
    try {
        ...
        match(JSON.parse(val))
            .with({ StrokesPlusHTMLWindow: {}}, ({ StrokesPlusHTMLWindow }) => {
                ...
                setHandle(handle);
                if (defaults.maximizeOnOpen) sp.WindowFromHandle(handle).Maximize();
            })
            .with({ action: "close" }, ({ action }) => {
...
function HtmlWindow(title, html, webviewMessageHandler, windowOnLoadScript = "") {
    if (typeof this[webviewMessageHandler] !== "function") {
        throw Error(`"${webviewMessageHandler}" is not a function`);
    }

    sp.HTMLWindow(title, html, webviewMessageHandler, windowOnLoadScript, "", true);
}

var html = template(String.raw`C:\Users\Public\shop\automation\demos\html\app.html`, {
